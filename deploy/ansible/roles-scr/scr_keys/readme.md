...
Every SCR playbook (`playbook_scr_initialize.yaml`,
`playbook_scr_db_test.yaml`, ...) invokes this role once for the
**source** system and once for the **destination** system before any
work runs, and then again with `cleanup: true` at the end to shred the
temporary key files.
...
## End-to-End Flow (in `playbook_scr_initialize.yaml`)

```
pre_tasks
├── Step 2  scr_keys  (function=source, granularity=workload)  ─► ansible_user + ansible_ssh_private_key_file on every source host
└── Step 2  scr_keys  (function=dest,   granularity=workload)  ─► ...on every destination host
...
See [../README.md](../README.md) for the full SCR framework and
[../scr_log_function/README.md](../scr_log_function/README.md) for the
logging/checkpoint model used around this role.
